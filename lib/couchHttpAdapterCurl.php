...
class couchHttpAdapterCurl extends couchHttpAdapterAbstract implements couchHttpAdapterInterface
{
     ...
     *            the request body. If it's an array or an object, $data is json_encode()d
     * @param string $content_type
     *            the content type of the sent data (defaults to application/json)
     ...
     * uses PHP cURL API
     *
     * @todo implemment exception for curl errors
     *
     * @param string $method
     ...
     *            additionnal parameters to send with the request
     * @param string|array|object $data
     *            request body
     * @param string $content_type
     *            the content type of the sent data (defaults to application/json)
     *
     * @return string false response on success, false on error
     *
     * @throws Exception
     */
     ...
     *
     * @param string $url
     *            CouchDB URL to store the file to
     * @param string $data
     *            data to send as the attachment content
     * @param string $content_type
    ...
    {
        static $allowFollowLocation;

        if ($allowFollowLocation !== null) {
            return $allowFollowLocation;
        }

        if (version_compare(PHP_VERSION, '5.4.0', '<')) { // php 5.4 lower
            $allowFollowLocation= ini_get('open_basedir') == '' && ini_get('safe_mode') !== 'Off';
        } else { // php 5.4 greater
            $allowFollowLocation= ini_get('open_basedir') == '';
        }
